...
> {
    ...
    private _store: UseBoundStore<StoreApi<TState & BaseActions<TState> & any>> | null = null;
    ...
    protected getEndpoint(append: string = ''): string {
        const base = typeof this.config.endpoint === 'function' 
     ...
     */
    public async getAll(params: any = {}, options: ApiOptions = {}): Promise<ListResponse<T> | T[] | null> {
     ...
     */
    public async getById(id: string | number, options: ApiOptions = {}): Promise<T | null> {
     ...
     */
    public async update(id: string | number, data: U, options: ApiOptions = {}): Promise<T | null> {
        const url = this.getEndpoint(`/${id}`);
        const mergedOptions = this.applyMiddleware({ 
            data,
            displayError: true,
     ...
     */
    public async delete(id: string | number, options: ApiOptions = {}): Promise<any> {
        const url = this.getEndpoint(`/${id}`);
        const mergedOptions = this.applyMiddleware({ 
            displayError: true, 
            displaySuccess: false,
    ...
    public async customAction<Res = any>(
        ...
        const mergedOptions = this.applyMiddleware({ displayError: true, ...options });

        switch(method) {
            case 'get': return await apiGet(url, mergedOptions);
            case 'post': return await apiPost(url, mergedOptions);
            case 'put': return await apiPut(url, mergedOptions);
            case 'delete': return await apiDelete(url, mergedOptions);
        }
    }
