...
export const GenericForm: React.FC<GenericFormProps> = ({
    ...
    const [dynamicOptions, setDynamicOptions] = useState<Record<string, InputOption[]>>({});
    ...
    useEffect(() => {
        fields.forEach(field => {
            if (field.type === 'select' && field.fetchOptions) {
                ...
                }).catch(err => console.error(`Failed to load options for ${field.name}`, err));
            }
        });
    }, [fields]);
    ...
    const validateField = (name: string, value: any, rules?: any): string | null => {
        ...
        if (rules.required && (value === '' || value === null || value === undefined)) return rules.message || 'Required';
        if (rules.min && typeof value === 'number' && value < rules.min) return `Min value is ${rules.min}`;
        if (rules.pattern && !rules.pattern.test(value)) return rules.message || 'Invalid format';
    ...
    return (
        <form onSubmit={handleSubmit} className={cn("space-y-4 p-4 h-full overflow-y-auto", className)}>
            {fields.map(field => {
                             ...
                             <div className="border border-dashed p-4 rounded-md text-center">
                                {val ? (
                                    <div className="relative w-full h-32 bg-gray-100 mb-2 rounded-md overflow-hidden">
                                        <img src={typeof val === 'string' ? val : URL.createObjectURL(val)} alt="preview" className="object-contain h-full w-full" />
                                        <button 
                                            type="button"
